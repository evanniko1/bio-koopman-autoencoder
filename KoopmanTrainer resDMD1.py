        ...
        for epoch in tqdm(range(self.num_epochs), desc="Training Epochs"):
            ...
            for batch_idx, data_list in enumerate(self.train_loader):
                ...
                ##################################                
                    ...
                    eigenvectors = eigenvectors.real  # Ensure eigenvectors are real (if K is real)
                    eigenvalues = eigenvalues.real  # If K is unitary, eigenvalues should be on the unit circle
                    Psi_X = Ytr[:, :-1, :]
                    Psi_Y = Ytr[:, 1:, :]
                    # reshape the Psi matrices
                    batch_size, num_steps, latent_dim = Ytr.shape
                    M = batch_size * (num_steps - 1)
                    Psi_X = Psi_X.reshape(batch_size * (num_steps - 1), latent_dim)
                    Psi_Y = Psi_Y.reshape(batch_size * (num_steps - 1), latent_dim)
                    # the weight matrix 
                    W = torch.eye(M, device=self.device)
                    # Compute Psi^* W Psi_X and Psi^* W Psi_Y
                    Psi_X_conj = torch.conj(Psi_X)  # Shape: (M, latent_dim)
                    Psi_Y_conj = torch.conj(Psi_Y)  # Shape: (M, latent_dim)


                    Psi_Y_conj_W_Psi_Y = Psi_Y_conj.T @ W @ Psi_Y  # Shape: (latent_dim, latent_dim) #1
                    # calculate Psi_X_conj_W_Psi_Y and then make conjugate transpose of the result 
                    Psi_X_conj_W_Psi_Y = Psi_X_conj.T @ W @ Psi_Y # Shape: (latent_dim, latent_dim) # 3
                    Psi_conj_T = torch.conj(Psi_X_conj_W_Psi_Y).T  # Shape: (latent_dim, latent_dim) # 2
                    Psi_X_conj_W_Psi_X = Psi_X_conj.T @ W @ Psi_X  # Shape: (latent_dim, latent_dim) # 4
                    ...
                    for i in range(num_eigenpairs):
                        ...
                        numerator = g_i_conj @ (Psi_Y_conj_W_Psi_Y - lambda_i * Psi_conj_T - lambda_i_conj * Psi_X_conj_W_Psi_Y + (abs(lambda_i)**2) *  Psi_X_conj_W_Psi_X) @ g_i
                        denominator = g_i_conj @ Psi_X_conj_W_Psi_X @ g_i

                        if torch.abs(denominator) < 1e-8:
                            residual_i = torch.tensor(0.0, device=self.device)
                        else:
                            residual_i = numerator / denominator

                        # Add the residual loss to the total loss
