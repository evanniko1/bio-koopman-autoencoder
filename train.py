...
def train(model, train_loader, lr, weight_decay, 
          ...
          nu=0.0, eta=0.0, backward=0, steps=1, steps_back=1, gradclip=1,policy='koopmanAE'):
    ...
    if policy == 'AE':
        '''
        Autoencoder training where we only take the model.encoder and model.decoder and test it against the the reconstruction
        '''
        for epoch in range(num_epochs):
            for batch_idx, data_list in enumerate(train_loader):
                model.train()
                out = model.encoder(data_list[0].to(device))
                out = model.decoder(out)
                loss = criterion(out, data_list[0].to(device))
